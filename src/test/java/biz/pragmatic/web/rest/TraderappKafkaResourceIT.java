package biz.pragmatic.web.rest;
...
import biz.pragmatic.config.KafkaProperties;
...
class TraderappKafkaResourceIT {
    ...
    void setup() {
        ...
        TraderappKafkaResource kafkaResource = new TraderappKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/traderapp-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/traderapp-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
